...
# Solution:  
1. `krypton4` content is `KSVVW BGSJD SVSIS VXBMN YQUUK BNWCU ANMJS`  
2. `found1` content is:  
```
...
Okay, so what is `Frequency Analysis`?  
...
Okay so we need to count what kind of characters are most common in ciphertext and also lists the bigrams and thigrams, lets write simple Python script. (LEVEL3.py)

7. Run the script to see which letter are occuring the most and which bigrams and trigrams are in the top results.
8. The results are as follow:
    `S` is the most common character, we can assume that this must map to the most common letter in english vocabulary which is `E`
    `'DS', 'JD'` are the most comon bigrams
    `'JDS` is the most common trigram -> we can assume that this is `THE`
9. So our initial mapping is: `{ 'S': 'e', 'J': 't', 'D': 'h'}`
10. After having this three characters done, we can start to see some missing characters and fill the mapping one by one until we get most of the characters.
11. I also used `https://www.thefreedictionary.com/words-containing-twoo` a lot to see what kind of letters I might be missing
12. In the end the mapping looks like this:
```
mapping = { 'S': 'e', 'J': 't', 'D': 'h', 'K': 'w', 'N': 'r', 'V': 'l', 'C': 'i', 'B': 'o', 'U': 's', 'I': 'v', 'W': 'd', 'Y': 'p', 'Q': 'a', 'M': 'u', 'E': 'g', 'G': 'n', 'X': 'f', 'T': 'm', 'Z': 'c', 'A': 'b' }
```
13. And the reversed ciphertext becomes:
`welld oneth eleve lfour passw ordis brute` -> `well done the level four password is brute`
14. We get `BRUTE`